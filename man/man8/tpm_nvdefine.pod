...
Read locality value, one byte in hex (starting with 0x) or decimal

Any combination of localities is valid:
...
=over 4
	...
	Locality three = 0x8
	Locality four  = 0x10
	All localities = 0x1f (default)


=back

Note that on an unmodified Linux system, only locality zero is implemented in the TPM
device driver.  This means that if your NVRAM area doesn't have the read locality zero
bit set, it will be unreadable from an application without device driver modification.

=item B<-W, --wlv>

Write locality value, one byte in hex (starting with 0x) or decimal
...
Note that on an unmodified Linux system, only locality zero is implemented in the TPM
device driver.  This means that if your NVRAM area doesn't have the write locality zero
bit set, it will be unwritable from an application without device driver modification.
